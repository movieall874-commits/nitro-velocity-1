...
async function boot(){try{await pool.query('ALTER TABLE saves ADD COLUMN IF NOT EXISTS engine INTEGER NOT NULL DEFAULT 0');await pool.query('ALTER TABLE saves ADD COLUMN IF NOT EXISTS nitro_level INTEGER NOT NULL DEFAULT 0');await pool.query('ALTER TABLE saves ADD COLUMN IF NOT EXISTS wins INTEGER NOT NULL DEFAULT 0');console.log('Database schema ready.')}catch(e){console.error('Database schema check failed:',e.message)}app.listen(PORT,()=>console.log(`Nitro Velocity API listening on :${PORT}`))}
boot();
